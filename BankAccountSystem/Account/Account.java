...
public abstract class Account {
	...
	private double beginningBalance, balance, averageBalance, totalWithdrawal,
			totalDeposit;
	private Customer customerReference;
	private List<Transaction> transactionArrayList = new ArrayList<>();
	// should have used immutable key
	...
	private void setCustomerAndTransactionMap() {
		customerAndTransactionMap.put(this.getAccountOwner(),
				this.transactionArrayList);
	}
	...
	protected void processTransaction(int transType, double amount, int transDay) {
		this.chooseTransactionOption(transType, amount);// call method to make
														// deposit or withdrawal

		// checks if balance is overdrawn
		if (this.getBalance() < 0) {

			// if balance is overdrawn, retrieve the status of the account from
			// Customer class using Customer reference
			String checkStatus = this.customerReference.getCustomerStatus();// get
																			// the
																			// customer
																			// status
			...
			if (checkStatus.equalsIgnoreCase("regular")) {
				...
				customerAndTransactionMap.get(this.customerReference).add(
						new Transaction(transType, transDay, amount,
			...
			this.transactionArrayList.add(new Transaction(transType, transDay,
					amount, " "));
		}
	}
	...
	private static void getAllTransaction(Customer customer) {
		...
		Collections.sort(customerAndTransactionMap.get(customer),
				Transaction.transactionDate);

		for (Transaction trans : (customerAndTransactionMap.get(customer))) {
			System.out
					.printf("Transaction Date: %s%nTransaction Type: %s%nTransaction Amount: $%.2f%n",
							trans.getTransDay(), trans.getTransType(),
							trans.getAmount());
	...
	// this prints a list of all customers and their transactions. Is for
	// project3, would remove in production.
	protected static void getAllCustomers() {
		for (Customer customer : (customerAndTransactionMap.keySet())) {
			// System.out.println(customer.getClass());
			System.out.printf("Name:   %s%nStatus: %s%n",
	...
	protected String getCustomerTransaction() {
		...
		Collections.sort(customerAndTransactionMap.get(this.customerReference),
				Transaction.transactionDate);

		for (Transaction trans : (customerAndTransactionMap
				.get(this.customerReference))) {
			newString
					.append(String.format("Transaction Date:   %s%n",
							trans.getTransDay()))
					.append(String.format("Transaction Type:   %s%n",
							trans.getTransType()))
					.append(String.format("Transaction Amount: $%.2f%n",
							trans.getAmount()));
			if (!trans.getMessage().equalsIgnoreCase(" ")) {

				newString.append(String.format("Message: %s%n",
						trans.getMessage()));
			}
	...
	// below doesn't really have a purpose yet just learning how to navigate
	// data structures
	/*
	 * private void printMap(){ for (Map.Entry<Customer, List<Transaction>>
	 * entry : customerAndTransactionMap.entrySet()) { Customer key =
	 * entry.getKey(); Object value = entry.getValue();
	 * System.out.printf("Key is: %s%nValue is: %s%n", key.toString(),
	 * value.toString()); } }
	 * 
	 * public void print() { printMap(); }
	 */
